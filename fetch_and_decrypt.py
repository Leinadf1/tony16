...
# URL del frida-server.xz nella repo originale
...
def ensure_decryptor_jar():
    ...
    local_jar_path = jar_name  # nella directory corrente

    if not os.path.exists(local_jar_path):
        print("Decryptor.jar not found. Re-building...")
        try:
            java_file = "Decryptor.java"  # deve essere nella root
            android_jar = os.path.join(os.environ.get("ANDROID_HOME", "/usr/local/lib/android/sdk"),
                                      "platforms", "android-33", "android.jar")
            d8 = os.path.join(os.environ.get("ANDROID_HOME", "/usr/local/lib/android/sdk"),
                             "build-tools", "37.0.0", "d8")  # usa la versione disponibile

            # Compila Decryptor.java in Decryptor.class
            subprocess.run(["javac", "--release", "8", "-cp", android_jar, java_file], check=True)
            # Usa d8 per creare classes.dex
            subprocess.run([d8, "--lib", android_jar, "--output", ".", "Decryptor.class"], check=True)

            # Crea il JAR con il dex
            import zipfile
    ...
    try:
        ...
        if "frida-server" not in frida_ps_out:
            ...
            if "No such file" in check_fs_err or "frida-server" not in check_fs_out:
                ...
                elif os.path.exists(FRIDA_SERVER_XZ):
...
if __name__ == "__main__":
    # Scarica frida-server se non presente
    ensure_frida_server()
    main()
